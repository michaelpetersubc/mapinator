...
for package in ["BlackBoxOptim", "Distributions", "ForwardDiff", "JSON", "Optim", "Quadrature", 
        "StatsPlots","DotEnv","MySQL","DBInterface","Tables","FLoops"]
    if INSTALL_PACKAGES
        Pkg.add(package)
    end
end
using  JSON, HTTP, Distributions, DotEnv, MySQL, DBInterface, Tables,FLoops
...
function bucket_estimate(assign::Array{Int64}, A::Matrix{Int64}, num, numsink,b::Matrix{Int64})
